    ...
    if (canWrite) {
      ...
      if (on) {
        action(ICONS.stop, t('Stop sharing'), false, () => {
          if (!this.session) return
          void stopSharing(this.session, this.store).then(() => {
